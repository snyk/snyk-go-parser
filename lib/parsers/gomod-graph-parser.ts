...
export function parseGoModGraph(goModGraphOutput: string,
                                projectName: string,
                                projectVersion: string = DEFAULT_INITIAL_VERSION): DepGraph {
  ...
  for (const line of goModGraphOutput.trim().split('\n')) {
    const [
        [parentName, parentVersion = DEFAULT_INITIAL_VERSION], [childName, childVersion],
    ] = parseGoModGraphLine(line);

    const parentPkg: PkgInfo = {name: parentName, version: parentVersion};
    const childPkg: PkgInfo = {name: childName, version: childVersion || DEFAULT_INITIAL_VERSION};
    const parentNodeId = parentName === rootPkgInfo.name ? DEFAULT_ROOT_NODE_NAME : `${parentName}@${parentVersion}`;
