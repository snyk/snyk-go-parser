...
it('simple example: gomod parsing', async () => {
  ...
  const goModGraph = parseGoModGraph(
    exampleGoMod,
    'github.com/spf13/jwalterweatherman',
  );
  const expectedGraph = createFromJSON(
    JSON.parse(load(path.join('gomod', 'simple', 'expected-graph.json'))),
  );
  expect(goModGraph.equals(expectedGraph, { compareRoot: true })).toBe(true);
});

it('pruning example: gomod parsing', async () => {
  const exampleGoMod = load(path.join('gomod', 'k8s-like', 'gomodgraph'));
  const goModGraph = parseGoModGraph(exampleGoMod, 'k8s.io/kubernetes');
  const expectedGraph = createFromJSON(
    JSON.parse(load(path.join('gomod', 'k8s-like', 'expected-graph.json'))),
  );
  expect(goModGraph.equals(expectedGraph, { compareRoot: true })).toBe(true);
});
...
it('empty example: gomod parsing', async () => {
  ...
  const expectedGraph = createFromJSON(
    JSON.parse(load(path.join('gomod', 'empty', 'expected-graph.json'))),
  );
  try {
    expect(goModGraph.equals(expectedGraph, { compareRoot: true })).toBe(true);
  } catch (e) {
...
it('prefix versioning support', async () => {
  const exampleGoMod = load(
    path.join('gomod', 'semver-prefixed', 'gomodgraph'),
  );
  const goModGraph = parseGoModGraph(exampleGoMod, '');
  const expectedGraph = createFromJSON(
    JSON.parse(
      load(path.join('gomod', 'semver-prefixed', 'expected-graph.json')),
    ),
  );
  try {
    expect(goModGraph.equals(expectedGraph, { compareRoot: true })).toBe(true);
